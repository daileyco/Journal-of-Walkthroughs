...
source("./02-Scripts/02-Helper-Functions/runMetropolisRandomWalk_v2.R")
...
###### curve(exp(-(tau/2)*x), from = 1, to = 30) 
...
params.proposal.tbl <- tibble(param.names = c(paste0("alpha", 0:1), 
                              ...
                              proposal.function = c(rep("rnorm", 10), 
                                                    "runif"), 
                              proposal.function.args = c(as.pairlist(lapply(1:2, function(x){list(n=1, sd=4)})), 
                                                         as.pairlist(lapply(1:4, function(x){list(n=1, sd=1.5)})),
                                                         as.pairlist(lapply(1:4, function(x){list(n=1, sd=1.5)})), 
                                                         list(list(n=1, 
...
# ### test gibbs
# test <- runMetropolisRandomWalk(chain.length = 100,
#                                 observed.data = od,
