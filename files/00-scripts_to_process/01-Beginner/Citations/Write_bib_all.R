...
if(!dir.exists("./References")){setwd("..")}

# List .bib files within "References" directory 
bib.files <- list.files(path="./References", pattern="\\.bib$", recursive = TRUE, full.names = TRUE)
...
write.table(unlist(bibs), file = "./references.bib", quote = FALSE, row.names = FALSE, col.names = FALSE)
